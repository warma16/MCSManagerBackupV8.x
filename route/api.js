...
router.ws('/flow',(ws,req)=>{
  var name=getPar('name',req.url)
  var server=serverModel.ServerManager().getServer(name)
  if(server){
    server.on('console',(dat)=>{
      ws.send(JSON.stringify({event:"console",data:dat}))
    })
    server.on("open", (server) => {
      ws.send(JSON.stringify({event: "open", data: server}))
    })
    server.on("error", (err) => {
